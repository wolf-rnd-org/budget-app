...
export function ExpenseReviewStep({ parsedData, initialInvoiceFile, initialBankFile, onBack, onSuccess, onCancel }: ExpenseReviewStepProps) {
  ...
  const [formData, setFormData] = React.useState({
    ...
    program_id: "",                 // <<< חדש: זה מה שנשלח לשרת
  ...
  React.useEffect(() => {
    ...
    if (auto && auto !== formData.program_id) {
      setFormData(prev => ({ ...prev, program_id: auto }));
    }
    // שימי לב: אין כאן תלות ב-formData.program_id כדי לא לטריגר רנדרים מיותרים
  }, [programs, currentProgramId]);
  // 2ב: איפוס קטגוריות *רק* כאשר ה-program_id השתנה באמת
  React.useEffect(() => {
    if (formData.program_id && prevProgramIdRef.current !== formData.program_id) {
      setFormData(prev => ({ ...prev, categories: [] }));
      prevProgramIdRef.current = formData.program_id;
    ...
    <div className="p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <AIAnalysisBanner
          status="ready"
          modelName="Gemini"
          // confidence={0.78} // אם אין, פשוט הסירי
          checklist={[
            !formData.supplier_name && "שם ספק חסר",
            !formData.business_number && "מספר עסק חסר",
            !formData.invoice_description && "תיאור החשבונית חסר",
            formData.amount <= 0 && "סכום חשוד/אפס",
            !formData.program_id && "פרויקט לא נבחר",
            formData.categories.length === 0 && "לא נבחרו קטגוריות",
              ...
              <div className="md:col-span-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 pr-10 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white [appearance:auto]"
                    required
                    disabled={programsLoading || programs.length === 0}
                  >
                    <option value="" disabled>
                      {programsLoading ? 'טוען פרויקטים…' : (programs.length ? 'בחרי פרויקט…' : 'אין פרויקטים משויכים')}
                    </option>
                    {programs.map(p => (
